...
class DAKOTAInputValidationError(InputValidationError):
    """Raise when the parser encounters an error while creating the input file of Dakota."""
...
def study_input_helper(input_params, structure, stop_at_first_error=False, flat_mode=False, version='6.2'):
    """Validate if the input dictionary for Dakota is valid.

    Return the dictionary (possibly with small variations: e.g. convert integer to float where necessary, recreate the
    proper structure if flat_mode is True, ...) to use as input parameters (use_parameters) for the dakota calculation.
    ...
    :param structure: the StructureData object used as input for Dakota dakota
    ...
    :param flat_mode: if True, instead of passing the dictionary of namelists,
        ...
        will return the correct dictionary to pass to the StudyCalculation,
    ...
    :raise DAKOTAInputValidationError:
    ...
    if not isinstance(input_params, dict):
        raise DAKOTAInputValidationError('input_params must be a dictionary')
            ...
            for key, value in content.items():
                ...
                if key in input_original_namelists:
                    ...
                    if stop_at_first_error:
                        raise DAKOTAInputValidationError(err_str)
                    else:
    ...
    xml_path = os.path.join(module_dir, f'INPUT_STUDY-{version}.xml')
    try:
        ...
        raise DAKOTAInputValidationError(
            f"Unknown Dakota version: {version}. Available versions: {', '.join(versions)};{add_str}"
        ) from exception

    # ========== List of known STUDY variables (from XML file) ===============
    ...
    for keyword in known_kws:
        ...
        try:
            ...
            # I ignore them
            pass
            # raise DAKOTAInputValidationError("Unable to find namelist for "
    ...
    # ====== List of known STUDY 'dimensions' (arrays) (from XML file) ===========
    ...
    for dim in known_dims:
        ...
        try:
            ...
            # I ignore them
            pass
            # raise DAKOTAInputValidationError("Unable to find namelist "
    ...
    # ====== List of known STUDY 'multidimensions' (arrays) (from XML file) ===========
        ...
        if keyword in valid_kws:
            ...
            try:
                ...
                if namelist_name in all_namelists:
                    ...
                    if stop_at_first_error:
                        raise DAKOTAInputValidationError(err_str) from exception
                    else:
                        errors_list.append(err_str)
                else:
    ...
    if errors_list:
        raise DAKOTAInputValidationError(f'Errors! {len(errors_list)} issues found:\n* ' + '\n* '.join(errors_list))
