...
class StudyParser(Parser):
    ...
    def validate_ionic(self, trajectory, parameters, logs):
        ...
        # BFGS fails twice in a row in which case DAKOTA will print that convergence is reached while it is not necessarily
    ...
    def is_ionically_converged(self, trajectory, except_final_scf=False):
        ...
        """
        from aiida_dakota.utils.defaults.calculation import study
        from aiida_dakota.utils.validation.trajectory import verify_convergence_trajectory

        relax_type = self.get_calculation_type()
        parameters = self.node.inputs.parameters.get_dict()
        threshold_forces = parameters.get('CONTROL', {}).get('forc_conv_thr', study.forc_conv_thr)
        threshold_stress = parameters.get('CELL', {}).get('press_conv_thr', study.press_conv_thr)
    ...
    def build_output_structure(self, parsed_structure):
        ...
        """
        from aiida_dakota.parsers.parse_raw import convert_qe2aiida_structure
    ...
    def get_extended_symmetries(self):
        ...
        from aiida_dakota.parsers.parse_raw.study import get_symmetry_mapping
