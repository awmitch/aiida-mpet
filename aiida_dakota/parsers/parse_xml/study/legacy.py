...
from aiida_dakota.parsers.parse_xml.legacy import xml_card_cell, xml_card_ions, xml_card_header, read_xml_card, \
...
def parse_study_xml_pre_6_2(xml_file, dir_with_bands):
    """Parse the content of XML output file written by `study.x` with the old schema-less XML format.
...
# CARD ELECTRIC_FIELD
    ...
    # CARD PLANE_WAVES
    parsed_data = copy.deepcopy(xml_card_planewaves(parsed_data, dom, 'study'))
    ...
    try:
        ...
        raise DAKOTAOutputParsingError(f'Error parsing tag K-POINT.{i + 1} inside {target_tags.tagName}.')
...
#             except Exception:
#                 raise DAKOTAOutputParsingError('Error parsing tag {}'.format(tagname)+\
...
# CARD BAND STRUCTURE INFO
    ...
    if units not in ['hartree']:
        raise DAKOTAOutputParsingError(f"Expected energy units in Hartree. Got instead {parsed_data['energy_units']}")
    ...
    if dir_with_bands:
        try:
            ...
            for i in range(parsed_data['number_of_k_points']):
                ...
                def read_bands_and_occupations(eigenval_n):
                    ...
                    if metric not in ['Hartree']:
                        raise DAKOTAOutputParsingError('Error parsing eigenvalues xml file, ' + \
            ...
            raise DAKOTAOutputParsingError(f'Error parsing card {tagname}: {exception.__class__.__name__} {exception}')
...
#                     raise DAKOTAOutputParsingError('Error while searching for LUMO.')
