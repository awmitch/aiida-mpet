...
from aiida_dakota.parsers.parse_xml.exceptions import XMLUnsupportedFormatError
...
def get_xml_file_version(xml):
    """Return the version of the Quantum ESPRESSO study.x XML output file.
...
def get_schema_filepath(xml):
    ...
    schema_filename = get_schema_filename(xml)  # if schema == 'study' else 'qes_cp_devel_030920.xsd'
...
def is_valid_post_6_2_version(xml):
    """Return whether the given XML object corresponds to an XML output file of Quantum ESPRESSO study.x post v6.2.
...
def is_valid_pre_6_2_version(xml):
    """Returns whether the given XML object corresponds to an XML output file of Quantum ESPRESSO study.x pre v6.2.
