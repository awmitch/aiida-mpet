...
from aiida_dakota.parsers import DAKOTAOutputParsingError
...
# the xml file of DAKOTA v5.0.x (version below not tested)
def read_xml_card(dom, cardname):
    try:
        ...
        print(e)
        raise DAKOTAOutputParsingError(f'Error parsing tag {cardname}')
...
def parse_xml_child_integer(tagname, target_tags):
    try:
        ...
        raise DAKOTAOutputParsingError(f'Error parsing tag {tagname} inside {target_tags.tagName}')
...
def str2bool(string):
    try:
        ...
        if string in true_items:
            ...
            raise DAKOTAOutputParsingError(f'Error converting string {string} to boolean value.')
    except Exception:
        raise DAKOTAOutputParsingError('Error converting string to boolean.')
...
def parse_xml_child_attribute_str(tagname, attributename, target_tags):
    try:
        ...
        raise DAKOTAOutputParsingError(
...
def xml_card_cell(parsed_data, dom):
    ...
    try:
        ...
        raise DAKOTAOutputParsingError(f'Error parsing tag {tagname} inside {target_tags.tagName} inside {cardname}.')
...
def xml_card_ions(parsed_data, dom, lattice_vectors, volume):
    ...
    try:
        ...
        raise DAKOTAOutputParsingError(f'Error parsing tag SPECIE.# inside {target_tags.tagName}.')
...
# if parsed_data['units_for_atomic_positions'] not in ['alat','bohr','angstrom']:
    ...
    try:
        ...
        raise DAKOTAOutputParsingError(f'Error parsing tag ATOM.# inside {target_tags.tagName}.')
...
def xml_card_planewaves(parsed_data, dom, calctype):
    if calctype not in ['study', 'cp']:
        raise ValueError("Input flag not accepted, must be 'cp' or 'study'")
    ...
    if 'hartree' not in units:
        if 'rydberg' not in units:
            raise DAKOTAOutputParsingError(f'Units {units} are not supported by parser')
    else:
        if 'hartree' in units:
...
def xml_card_symmetries(parsed_data, dom):
    ...
    if metric not in ['crystal']:
        raise DAKOTAOutputParsingError(f'Error parsing attribute {attrname},' + \
...
def xml_card_exchangecorrelation(parsed_data, dom):
    ...
    if parsed_data[tagname.lower()]:  # if it is a plus U calculation, I expect more infos
        ...
        try:
            ...
            raise DAKOTAOutputParsingError('Error parsing tag '+\
