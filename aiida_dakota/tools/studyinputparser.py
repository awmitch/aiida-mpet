# -*- coding: utf-8 -*-
"""Utilities to parse Quantum ESPRESSO study.x input files into AiiDA nodes or builders."""
import copy
import re
import numpy as np
...
from qe_tools.parsers import StudyInputFile as BaseStudyInputFile
...
class StudyInputFile(StructureParseMixin, BaseStudyInputFile):
    """Parser of Quantum ESPRESSO study.x input file into AiiDA nodes.

    .. note:: This mixes in :class:`~aiida_dakota.tools.base.StructureParseMixin` which adds the functionality
...
def create_builder_from_file(input_folder, input_file_name, code, metadata, pseudo_folder_path=None):
    """Create a populated process builder for a `StudyCalculation` from a standard DAKOTA input file and pseudo (upf) files.
    ...
    :return: a builder instance for StudyCalculation
    """
    StudyCalculation = CalculationFactory('dakota.study')

    builder = StudyCalculation.get_builder()
    ...
    with input_folder.open(input_file_name) as input_file:
        parsed_file = StudyInputFile(input_file.read())
    ...
    for namelist, blocked_key in StudyCalculation._blocked_keywords:  # pylint: disable=protected-access
