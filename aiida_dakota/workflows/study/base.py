# -*- coding: utf-8 -*-
"""Workchain to run a Quantum ESPRESSO study.x calculation with automated error handling and restarts."""
...
from aiida_dakota.calculations.functions.create_kpoints_from_distance import create_kpoints_from_distance
from aiida_dakota.common.types import ElectronicType, SpinType
from aiida_dakota.utils.defaults.calculation import study as qe_defaults
from aiida_dakota.utils.mapping import update_mapping, prepare_process_inputs
from aiida_dakota.utils.pseudopotential import validate_and_prepare_pseudos_inputs
from aiida_dakota.utils.resources import get_default_options, get_study_parallelization_parameters
from aiida_dakota.utils.resources import cmdline_remove_npools, create_scheduler_resources

from ..protocols.utils import ProtocolMixin

StudyCalculation = CalculationFactory('dakota.study')
...
def validate_pseudo_family(value, _):
    ...
    if value:
        ...
        warnings.warn('`pseudo_family` is deprecated, use `study.pseudos` instead.', AiidaDeprecationWarning)


class StudyBaseWorkChain(ProtocolMixin, BaseRestartWorkChain):
    """Workchain to run a Quantum ESPRESSO study.x calculation with automated error handling and restarts."""

    # pylint: disable=too-many-public-methods, too-many-statements

    _process_class = StudyCalculation
    ...
    def define(cls, spec):
        ...
        spec.expose_outputs(StudyCalculation)
    ...
    def get_protocol_filepath(cls):
        ...
        from ..protocols import study as study_protocols
        return files(study_protocols) / 'base.yaml'
    ...
    def get_builder_from_protocol(
        ...
        **_
        ...
        :param code: the ``Code`` instance configured for the ``dakota.study`` plugin.
        ...
        """
        from aiida_dakota.workflows.protocols.utils import get_starting_magnetization
        ...
        parameters = inputs['study']['parameters']
        ...
        builder.study['code'] = code
        builder.study['pseudos'] = pseudo_family.get_pseudos(structure=structure)
        builder.study['structure'] = structure
        builder.study['parameters'] = orm.Dict(dict=parameters)
        builder.study['metadata'] = inputs['study']['metadata']
        if 'parallelization' in inputs['study']:
            builder.study['parallelization'] = orm.Dict(dict=inputs['study']['parallelization'])
    ...
    def setup(self):
        ...
        self.ctx.inputs = AttributeDict(self.exposed_inputs(StudyCalculation, 'study'))
    ...
    def validate_kpoints(self):
        ...
        try:
            ...
            inputs = {
                'structure': self.inputs.study.structure,
    ...
    def run_init(self):
        """Run an initialization `StudyCalculation` that will exit after the preamble.
        ...
        inputs = prepare_process_inputs(StudyCalculation, inputs)
        running = self.submit(StudyCalculation, **inputs)
    ...
    def inspect_init(self):
        """Use the initialization `StudyCalculation` to determine the required resource and parallelization settings."""
        ...
        parallelization = get_study_parallelization_parameters(calculation, **self.ctx.automatic_parallelization)
    ...
    def sanity_check_insufficient_bands(self, calculation):
        ...
        """
        from aiida_dakota.utils.bands import get_highest_occupied_band
    ...
    @process_handler(priority=590, exit_codes=[
        StudyCalculation.exit_codes.ERROR_COMPUTING_CHOLESKY,
    ])
    ...
    @process_handler(priority=580, exit_codes=[
        StudyCalculation.exit_codes.ERROR_OUT_OF_WALLTIME,
    ])
    ...
    @process_handler(
        priority=570, exit_codes=[
            StudyCalculation.exit_codes.ERROR_IONIC_CONVERGENCE_REACHED_EXCEPT_IN_FINAL_SCF,
        ]
    )
    ...
    @process_handler(
        priority=560,
        exit_codes=[
            StudyCalculation.exit_codes.ERROR_IONIC_CONVERGENCE_NOT_REACHED,
            StudyCalculation.exit_codes.ERROR_IONIC_CYCLE_EXCEEDED_NSTEP,
            StudyCalculation.exit_codes.ERROR_IONIC_CYCLE_BFGS_HISTORY_FAILURE,
            StudyCalculation.exit_codes.ERROR_IONIC_CYCLE_BFGS_HISTORY_AND_FINAL_SCF_FAILURE,
        ]
    )
    ...
    @process_handler(
        priority=550,
        exit_codes=[
            StudyCalculation.exit_codes.ERROR_IONIC_CYCLE_ELECTRONIC_CONVERGENCE_NOT_REACHED,
            StudyCalculation.exit_codes.ERROR_IONIC_CONVERGENCE_REACHED_FINAL_SCF_FAILED,
        ]
    )
    ...
    @process_handler(priority=410, exit_codes=[
        StudyCalculation.exit_codes.ERROR_ELECTRONIC_CONVERGENCE_NOT_REACHED,
    ])
    ...
    @process_handler(priority=420, exit_codes=[
        StudyCalculation.exit_codes.WARNING_ELECTRONIC_CONVERGENCE_NOT_REACHED,
    ])
