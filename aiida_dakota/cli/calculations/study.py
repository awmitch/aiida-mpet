# -*- coding: utf-8 -*-
"""Command line scripts to launch a `StudyCalculation` for testing and demonstration purposes."""
import click
...
def launch_calculation(
    ...
    if dry_run:
        if daemon:
            ...
            # would be confusing ("Submitted StudyCalculation<None> to the daemon")
            raise click.BadParameter('cannot send to the daemon if in dry_run mode', param_hint='--daemon')
        inputs['metadata']['store_provenance'] = False
        inputs['metadata']['dry_run'] = True

    launch.launch_process(CalculationFactory('dakota.study'), daemon, **inputs)
