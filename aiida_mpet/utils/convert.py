...
def conv_to_python(val, quote_strings=True):
    """Convert a python value to a format suited for python input.

    :param val: the value to be read and converted to a Python-friendly string.
    """
    import numpy
...
def convert_input_to_namelist_entry(key, val, mapping=None):
    ...
    if isinstance(val, dict):
        ...
        for elemk, itemval in val.items():
            ...
            if key == 'keywords':
                ...
                list_of_strings.append((idx, f'  {key} = {conv_to_python(itemval)}\n'))
    ...
    # Single value
        ...
        return f'  {key} = {conv_to_python(val)}\n'