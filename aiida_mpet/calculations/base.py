# -*- coding: utf-8 -*-
"""Defines a `CalcJob` base class for `aiida-mpet`.

The custom `CalcJob` base class automatically sets the `invalidates_cache` attribute of exit codes based on the status
integer. All `CalcJob` implementations in `aiida-mpet` must use this base class, not `aiida.engine.CalcJob`.
"""
...
class CalcJobProcessSpec(_BaseCalcJobProcessSpec):
    """Process spec for `aiida-mpet` `CalcJob` classes.
...
class CalcJob(_BaseCalcJob):  # pylint: disable=abstract-method
    """Custom `CalcJob` class for `aiida-mpet` calculations."""
